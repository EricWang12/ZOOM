...
def show_landmarks(img, lmks, out_name, circle_size=3, color=(255, 0, 0), figsize=(10,8), is_copy=True):
    """
    ...
    """
    img_lmks = set_circles_on_img(img, lmks, circle_size, color, is_copy)
    # breakpoint()
    img_lmks = cv2.cvtColor(img_lmks, cv2.COLOR_BGR2RGB)
    cv2.imwrite(out_name, img_lmks)
