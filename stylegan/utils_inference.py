...
from detector_attack.lib.models import get_face_alignment_net, get_cls_net
from detector_attack.lib.config import config, config_imagenet, merge_configs

from matplotlib import pyplot as plt
from utils_landmarks import show_landmarks


def get_model_by_name(model_name, root_models_path='hrnetv2_models', prefix='HR18-', model_type='landmarks', device='cuda:4'):
...
def get_lmks_by_img(model, img,  out_name, output_size=(256, 256), rot=0):
#     img = np.array(Image.open(image_path).convert('RGB'), dtype=np.float32)
    ...
    show_landmarks(img_crop_new, (get_preds(pred)*4).cpu().numpy(), out_name)
...
def transform_pixel(point, center, scale, output_size, invert=0, rot=0):
    # Transform pixel location to different reference
    t = get_transform(center, scale, output_size, rot=rot)
    if invert:
        t = np.linalg.inv(t)
    new_pt = np.array([point[0] - 1, point[1] - 1, 1.]).T
